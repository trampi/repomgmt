...
class RepositoryTest < ActiveSupport::TestCase
  ...
  test 'a repository should have a first commit' do
    ...
    assert_equal expected_date, repository.first_commit.date
  end
  ...
  test 'a repository should have a last commit' do
    ...
    assert_equal expected_date, repository.last_commit.date
  end

  test 'a repository should have url' do
    # url prefix is set in global_setup.rb
    assert_equal 'ssh://gitolite@127.0.0.1/repository_one', repositories(:repository_one).url
  end

  test 'a repository should have tags' do
    assert_equal 0, repositories(:repository_one).tags.count
  end

  test 'a repository should have branches' do
    assert_equal 1, repositories(:repository_one).branches.count
  end

  test 'a repository should have commits on dates' do
    date = Date.new(2014, 8, 12)
    commit_date_hash = repositories(:repository_one).commits_per_day[0]
